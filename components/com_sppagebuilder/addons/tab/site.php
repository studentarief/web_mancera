	...
	public function css()
	{
		...
		foreach ($mediaQueries as $size => $media) {
			...
			if (is_object($nav_position) && isset($nav_position->$size)) {
				...
				$newNavWidth = is_object($navWidth) && isset($navWidth->$size) ? $navWidth->$size : $navWidth;			
				...
				$isNavJustified = (is_object($navJustified) && isset($navJustified->$size)) ? $navJustified->$size : $navJustified;
				...
				$newGutter	 = is_object($navGutter) ? ( empty($navGutter->$size) ? 0 : $navGutter->$size ) : $navGutter;
				$newNavWidth = is_object($navWidth) ? $navWidth->$size : $navWidth;
				...
				$isNavJustified = (is_object($navJustified) && $navJustified->$size) ? $navJustified->$size : $navJustified;
		...
		if (is_object($nav_position)) {					
			...
			$newNavWidth = is_object($navWidth) ? $navWidth->xl : $navWidth;
			...
			$isNavJustified = (is_object($navJustified) && $navJustified->xl) ? $navJustified->xl : $navJustified;
			...
			$newGutter 	 = is_object($navGutter) ? (empty($navGutter->xl) ? 0 : $navGutter->xl) : $navGutter;	
			$newNavWidth = (is_object($navWidth) && $navWidth->xl) ? $navWidth->xl : $navWidth;
